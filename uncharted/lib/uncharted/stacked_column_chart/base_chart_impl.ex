defimpl Uncharted.StackedColumnChart, for: Uncharted.BaseChart do
  alias Uncharted.BaseChart
  alias Uncharted.StackedColumnChart.{Column, ColumnSection, Dataset, Section}
  ...
  def column_sections(%Uncharted.BaseDatum{values: values} = data, max, sections) do
    ...
    |> Enum.map(fn {value, index} ->
      ...
      %Section{label: section_label, fill_color: fill_color} = sections
      |> Enum.with_index()
